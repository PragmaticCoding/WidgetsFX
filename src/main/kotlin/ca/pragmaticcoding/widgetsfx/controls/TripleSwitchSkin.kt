...
class TripleSwitchSkin(control: TripleSwitch) : SkinBase<TripleSwitch>(control) {
    ...
    init {
        ...
        control.valueProperty()
            .bind(Bindings.createObjectBinding(this::checkValue, theToggles.selectedToggleProperty()))
    ...
    private fun labelBox(): Region = HBox(20.0).apply {
        val labels =
            listOf(
                toggleLabel(skinnable.labelLeft),
                toggleLabel(skinnable.labelCentre),
                toggleLabel(skinnable.labelRight)
            )
        children += labels
